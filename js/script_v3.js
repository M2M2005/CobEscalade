...
async function generatePDF(SHEET_ID, YEAR, NAME) {
  try {
    ...
    const imgPointCalcule = await loadImageAsBase64('img/point_calcule.png');
...
async function tableauTopo(SHEET_ID) {
  ...
  const cotations = data.values[8] || [];
    // Calculer le max de dégaines automatiquement
    let maxDegaines = 0;
    for (let i = 0; i < degaines.length; i++) {
        const nbDeg = parseInt(degaines[i]) || 0;
        if (nbDeg > maxDegaines) maxDegaines = nbDeg;
    }
    console.log("Max dégaines calculé:", maxDegaines);
  const body = [];
  ...
  for (let couloir = 0; couloir < nbCouloirs; couloir++) {
    ...
    voiesCouloir.forEach((voie, index) => {
      ...
      for (let i = 1; i <= maxDegaines; i++) {
        console.log("Voie:", voie.degaines);
        if (i < voie.degaines || i === maxDegaines) {
          row.push({text: "", alignment: "center", style: "tableBody"});
        } else {
          row.push({text: "", alignment: "center", style: "tableHeader"});
        }
      }
